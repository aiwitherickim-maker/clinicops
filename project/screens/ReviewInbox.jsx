/* Staff Review Inbox — list left, full detail right with actions. */

function ReviewInbox({ inbox, onResolve, onAssign }) {
  const [selectedId, setSelectedId] = React.useState(inbox[0]?.id);
  ...
  const flashToast = (text, tone = "sage") => {
    setToast({ text, tone });
        ...
        {selected && (
          <div className="card" style={{ position: "relative" }}>
            ...
            <div className="detail-section">
              ...
              <div className="detail-quote">"{selected.message}"</div>
            ...
            <div className="detail-actions">
              <Button variant="ghost" size="sm" onClick={() => flashToast("Draft approved — sent to send queue.", "green")}>
                <IconCheckCircle size={14} /> Approve draft
              </Button>
              <Button variant="ghost" size="sm">
                <IconEdit size={14} /> Edit draft
              </Button>
              <Button variant={selected.risk === "high" ? "sage" : "secondary"} size="sm"
                onClick={() => { onAssign && onAssign(selected); flashToast(`Assigned to ${selected.task.assignee}`, "sage"); }}>
                <IconStethoscope size={14} /> Assign to {selected.routeTo}
              </Button>
              <Button variant="ghost" size="sm" onClick={() => flashToast("Escalated to clinical lead.", "amber")}>
                <IconAlert size={14} /> Escalate
              </Button>
              <div className="spacer" />
              <Button variant="secondary" size="sm" onClick={() => { onResolve && onResolve(selected); flashToast("Marked resolved.", "green"); }}>
                Mark resolved
              </Button>
            </div>
...
function InboxCard({ m, selected, onSelect }) {
  return (
    <button className={`inbox-card${selected ? " selected" : ""}`} onClick={onSelect}>
      <div className="ic-head">
        <Avatar initials={m.initials} tone={m.iconTone === "red" ? "red" : m.iconTone === "amber" ? "amber" : "sage"} size={28} />
        <span className="ic-name">{m.patient}</span>
        <span className="ic-time">{m.received}</span>
      </div>
      <div className="ic-msg">"{m.message}"</div>
