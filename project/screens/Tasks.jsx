/* Tasks — table view of AI-created tasks awaiting human action. */
...
function Tasks() {
  ...
  const approve = (id) => {
    setTasks((list) => list.map((t) => t.id === id ? { ...t, status: "Open", statusTone: "neutral" } : t));
  };
...
function TaskRow({ t, onApprove }) {
  return (
    <tr>
      ...
      <td>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          {t.status === "Pending approval" && (
            <button onClick={onApprove} style={{ background: "transparent", border: "none", color: "var(--sage-deep)", cursor: "pointer", fontSize: 11.5, fontWeight: 700 }}>
              Approve →
            </button>
          )}
        </div>
      ...
      <td>
        <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 12.5, color: t.due === "Today" ? "#A03A2D" : "var(--fg2)", fontWeight: t.due === "Today" ? 700 : 500 }}>
