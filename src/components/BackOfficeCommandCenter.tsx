...
export function BackOfficeCommandCenter() {
  ...
  const respondTo = (text: string) => {
    ...
    setTimeout(() => {
      ...
      let newAction: CommandAction | null = null;
      ...
      if (lc.includes('morning') || lc.includes('worklist')) {
        ...
        resp = "I drafted billing follow-ups for the 1 patient with an open cost question. Maria Alvarez is assigned, pending your approval.";
        flash(['ac-2', 'ac-3']);
      } else if (lc.includes('clinic') || lc.includes('clinical')) {
        resp = "1 item is in the clinician review queue — Maya Thompson's post-injection eye pain message. Routed to Dr. Sarah Lee, urgent.";
        flash(['ac-1']);
