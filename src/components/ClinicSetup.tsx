...
import { CLINIC, STAFF, REVIEW_RULES, KNOWLEDGE_SOURCES, SAFETY_SETTINGS } from '@/data/mockClinic';
import type { SafetySetting, ClinicProfile } from '@/types';
import { Button, Badge, IconTile, Toggle, PrincipleBanner } from './Primitives';
import {
  IconBuilding, IconUsers, IconRoute, IconBook, IconShieldCheck,
  IconArrowRight, IconPlus, IconCheckCircle, IconMoreH, IconChevronRight,
} from './Icons';

export function ClinicSetup() {
  const [safety, setSafety] = useState<SafetySetting[]>(SAFETY_SETTINGS);
            ...
            <div>
              <h2>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                  <IconBuilding size={16} style={{ color: 'var(--sage-deep)' }} /> Clinic profile
                </span>
              </h2>
        ...
        <div className="card">
          <div className="card-head">
            <div>
              <h2>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                  <IconUsers size={16} style={{ color: 'var(--sage-deep)' }} /> Staff roles
                </span>
              </h2>
      ...
      <div className="card">
        <div className="card-head">
          <div>
            <h2>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                <IconRoute size={16} style={{ color: 'var(--sage-deep)' }} /> Human review rules
              </span>
            </h2>
      ...
      <div className="card">
        <div className="card-head">
          <div>
            <h2>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                <IconBook size={16} style={{ color: 'var(--sage-deep)' }} /> Approved knowledge sources
              </span>
            </h2>
      ...
      <div className="card">
        <div className="card-head">
          <div>
            <h2>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                <IconShieldCheck size={16} style={{ color: 'var(--sage-deep)' }} /> Safety settings
              </span>
            </h2>
          ...
          {safety.map((s) => (
            <div className="toggle-row" key={s.id}>
              ...
              <Toggle
                on={s.on}
                locked={s.locked}
                onChange={(v) => setSafety((list) => list.map((x) => x.id === s.id ? { ...x, on: v } : x))}
              />
            </div>
          ))}
        </div>
