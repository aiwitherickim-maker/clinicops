...
import { DASH_METRICS, WORKLIST, COMMAND_CHIPS } from '@/data/mockClinic';
import type { WorklistItem } from '@/types';
import { Button, Badge, RiskPill, IconTile, PrincipleBanner } from './Primitives';
import {
  IconActivity, IconPlus, IconSparkles, IconArrowRight, IconChevronRight,
  IconCalendar, IconPill, IconDollar, IconUser, IconBot, IconAlert,
} from './Icons';
        ...
        <div className="card">
          <div className="card-head">
            ...
            <Button variant="ghost" size="sm" onClick={() => onNavigate('inbox')}>
              View all messages <IconChevronRight size={13} />
            </Button>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div className="card">
            ...
            <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <GlanceRow iconKey="user"     tone="sage" title="3 staff online"              sub="Dr. Sarah Lee · Maria Alvarez · Sam Park" />
            </div>
          </div>
...
function WorklistRow({ item, onAction }: { item: WorklistItem; onAction: () => void }) {
  return (
    <div className="worklist-row" onClick={onAction}>
      ...
      <div className="right-action">
        <Button
          variant={item.risk === 'high' ? 'sage' : 'secondary'}
          size="sm"
...
function GlanceRow({ iconKey, tone, title, sub }: { iconKey: string; tone: string; title: string; sub: string }) {
  return (
    <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
      <IconTile tone={tone as 'sage' | 'amber' | 'cream'} iconKey={iconKey} size="sm" />
      <div>
