...
import { CLINIC, NAV } from '@/data/mockClinic';
import { ICON_BY_KEY, IconStethoscope, IconSearch, IconBell, IconRefresh } from './Icons';
...
export function Sidebar({ active, onSelect, reviewCount, taskCount }: SidebarProps) {
  return (
    <aside className="sidebar">
      ...
      <nav>
        {NAV.map((item) => {
          const NavIcon = ICON_BY_KEY[item.iconKey] || IconStethoscope;
          const isActive = item.key === active;
          let cnt: number | undefined = item.count;
          ...
          const cls = [
            'nav-item',
            isActive ? 'active' : '',
            urgent && !isActive ? 'urgent' : '',
            isActive && urgent ? 'active urgent' : '',
          ].filter(Boolean).join(' ');
          return (
    ...
    <div className="topbar">
      ...
      <div className="right">
        <div className="search">
          <IconSearch size={15} />
          <input placeholder="Search patients, tasks, messages…" readOnly />
